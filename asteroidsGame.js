...
function calcJump() {
  if (asteroids[2][currentPosition] === 1) { //check if the asteroid about to be "jumped" to is a valid asteroid
    score++; 
    document.getElementById("score").innerHTML = "SCORE: " + score; //refresh the score in the HTML
    console.log("GOOD JUMP!");
  } 
  else { //asteroid jumped to an Error Asteroid (NOTE: if adding asteroids of alternate point value, this will need to be reworked)
    lives--;
  ...
  if(lives == 0) { 
    ...
    setTimeout(function () {
      ...
      else { //if the user chose not to Play Again, disable robot movement
...
function showGame() {
  ...
  for (let i = 0; i < asteroids.length; i++) {
    for (let j = 0; j < asteroids[i].length; j++) {
      ...
      if (i != 3) {
        if (asteroids[i][j] === -1) { //-1 means Error Asteroid
          var asteroidToDraw = document.getElementById("Error Asteroid");
        } 
        else { //Non -1 Value means Valid Asteroid
...
function showRobot(x, y) {
  ...
  if (currentPosition == 0) { //draw the robot in the left col
    //The drawImage() method works as following:
    //ctx.drawImage(image to display, x axis positon, y axis position, image width, image height)
    ctx.drawImage(robot, 300, 500, 100, 100);
  }         
  if (currentPosition == 1) { //draw the robot in the middle col
    //The drawImage() method works as following:
    //ctx.drawImage(image to display, x axis positon, y axis position, image width, image height)
    ctx.drawImage(robot, 500, 500, 100, 100);
  }
  if (currentPosition == 2) { //draw the robot in the right col
    //The drawImage() method works as following:
    //ctx.drawImage(image to display, x axis positon, y axis position, image width, image height)
